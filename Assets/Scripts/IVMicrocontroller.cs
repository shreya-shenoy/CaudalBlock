...
/// <summary>
/// The Microcontroller is contained within the SMMARTS_SDK namespace.
/// </summary>
namespace SMMARTS_SDK
{
    ...
    ///	
    /// </summary>
    public class IVMicrocontroller : MonoBehaviour
    {
        ...
        public static IVMicrocontroller ME;
        ...
        void AttemptPortOpening(string portName)
        {
            ...
            try
            {
                ...
                microcontrollerPort.Write("=");//IV SPECIFIC
...
namespace SMMARTS_SDK
{
    // This is a special version of the microconroller that can be used with the IV trainer.
    // it handshakes with the arduino with a different character, which tells the arduino to send more outputs:
    // these new outputs have to do with additional A/D channels for traction, tourniquet, and wrist slap sensing.
    ...
    public class IVMicrocontroller : MonoBehaviour
    {
        ...
        [SerializeField]
        private float tourniquetPressure;
        [SerializeField]
        private float slapPressure;
        [SerializeField]
        private float tractionPressure;
            ...
            try
            {
                if (String.IsNullOrEmpty(com_Port))
                {
                    ...
                    foreach (string portname in ports)
                    {
                        ...
                        // port.Write("-"); // default SMARTS MC handshake.
                        port.Write("="); // special handshake for IV Trainer
                    ...
                    try
                    {
                        ...
                        if (!String.IsNullOrEmpty(inString) && (inString.Substring(0, 1)).CompareTo("$") == 0 && (inString.Substring(inString.Length - 1)).CompareTo("#") == 0)
                        {
                            ...
                            try
                            {
                                ...
                                if (SplitArray.Length == 7) // this is the IV trainer.
                                {
                                    tui_Button = (int.Parse(SplitArray[0]) > 0);
                                    usProbePressureNotchSide = int.Parse(SplitArray[1]);
                                    usProbePressureFlatSide = int.Parse(SplitArray[2]);
                                    valvePressure = float.Parse(SplitArray[3]);
                                    tourniquetPressure = float.Parse(SplitArray[4]);
                                    slapPressure = float.Parse(SplitArray[5]);
                                    tractionPressure = float.Parse(SplitArray[6]);
                                    syringeValvePresent = true;
                                    dataReceived = true;
                                }
                            }
